...
async def app_validation_exception_handler(
    ...
    error_response = ErrorResponse(
        detail=[ErrorDetail(loc=[], msg=str(exc), type="validation_error")],
        timestamp=datetime.now(UTC),
        path=str(request.url.path),
        status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
    )
...
async def business_logic_exception_handler(
    ...
    error_response = ErrorResponse(
        detail=[ErrorDetail(loc=[], msg=str(exc), type=error_type)],
        timestamp=datetime.now(UTC),
        path=str(request.url.path),
...
async def authorization_exception_handler(
    ...
    error_response = ErrorResponse(
        detail=[ErrorDetail(loc=[], msg=str(exc), type="authorization_error")],
        timestamp=datetime.now(UTC),
        path=str(request.url.path),
...
async def resource_not_found_exception_handler(
    ...
    error_response = ErrorResponse(
        detail=[ErrorDetail(loc=[], msg=str(exc), type="not_found_error")],
        timestamp=datetime.now(UTC),
        path=str(request.url.path),
...
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    error_response = ErrorResponse(
        detail=[ErrorDetail(loc=[], msg=str(exc.detail), type=error_type)],
        timestamp=datetime.now(UTC),
        path=str(request.url.path),
